...
class TensorBoardLoggerExpanded(TensorBoardLogger):
    ...
    def __init__(self, hparam):
        super().__init__(hparam.log.tensorboard_dir,
                         name=hparam.name,
    ...
    def plot_spectrogram_to_numpy(self, y, y_noisy, y_recon, eps_error,
                                  y_recon_allstep, alignment, step):
        ...
        name_list = [
            'y_recon_allstep', 'y', f'y_noisy(Diffstep_{step})', 'y_recon',
            'error_recon'
        ]
        for i, yy in enumerate(
            [y_recon_allstep, y, y_noisy, y_recon, eps_error]):
            ax = plt.subplot(6, 1, i + 2)
            ax.set_title(name_list[i])
            plt.imshow(
                rosa.amplitude_to_db(self.stftmag(yy).numpy(),
                                     ref=np.max,
                                     top_db=80.),
                #vmin = -20,
                vmax=0.,
    ...
    def log_spectrogram(self, y, y_noisy, y_recon, eps_error, y_recon_allstep,
        ...
        spec_img = self.plot_spectrogram_to_numpy(y, y_noisy, y_recon,
                                                  eps_error, y_recon_allstep,
                                                  alignment, diff_step)
    ...
    def log_audio(self, y, y_noisy, y_recon, y_recon_allstep, epoch):
        ...
        for n, yy in zip(name_list, [y, y_noisy, y_recon, y_recon_allstep]):
            self.experiment.add_audio(n, yy, epoch,
