...
class WaveGradNN(BaseModule):
    ...
    def __init__(self, hparams):
        ...
        upsampling_in_sizes = [hparams.wavegrad.upsample.preconv_channel] +hparams.wavegrad.upsample.out_channels[:-1]
        # upsampling_in_sizes=list(upsampling_in_sizes)
        self.ublocks = torch.nn.ModuleList([
            UBlock(in_channels=in_size,
    ...
    def forward(self, yn, hidden_rep, noise_level):
        ...
        # print(f"nn_large.py: yn.shape: {yn.shape}")
        yn = yn.unsqueeze(1)
        # print(f"nn_large.py: yn.shape: {yn.shape}")
