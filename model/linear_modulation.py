...
class PositionalEncoding(BaseModule):
    ...
    def forward(self, noise_level):
        ...
        exponents = torch.arange(
            half_dim, dtype=torch.float32).to(noise_level) / float(half_dim)
        exponents = 1e-4**exponents

        exponents = self.linear_scale * noise_level.unsqueeze(
            1) * exponents.unsqueeze(0)
        # self.linear_scale 5000,noise_leveltorch.Size([1]), exponents torch.Size([16])
        # self.linear_scale 5000,noise_level torch.Size([8, 8]), exponents torch.Size([16])
        # input(
        #     f"self.linear_scale {self.linear_scale},noise_level {noise_level.shape}, exponents {exponents.shape}"
        # )
...
class FeatureWiseLinearModulation(BaseModule):
    ...
    def __init__(self, in_channels, out_channels, input_dscaled_by,
        ...
        self.scale_conv = Conv1dWithInitialization(in_channels=in_channels,
                                                   out_channels=out_channels,
                                                   kernel_size=3,
                                                   stride=1,
                                                   padding=1)
        self.shift_conv = Conv1dWithInitialization(in_channels=in_channels,
                                                   out_channels=out_channels,
                                                   kernel_size=3,
                                                   stride=1,
                                                   padding=1)
    ...
    def forward(self, x, noise_level):
        ...
        outputs = outputs + self.positional_encoding.forward(
...
class FeatureWiseAffine(BaseModule):
    ...
    def forward(self, x, scale, shift):
        ...
        return torch.nan_to_num(outputs, posinf=65504, neginf=-65504)
